...
namespace UnitTest
{
    ...
    public class Evaluator
    {
        ...
        /// 静态方法的执行字符串名称   
        /// </summary>   
        private const string staticMethodName = "Func";
        /// <summary>   
        ...
        private void ConstructEvaluator(EvaluatorItem[] items)
        {
            ...
            cp.ReferencedAssemblies.Add("system.io");              //添加程序集 system.dll 的引用   
            ...
            code.AppendLine("using static System.IO.Directory;");

            code.AppendLine("namespace Asnical ");                  //生成代码的命名空间为Asnical，和本代码一样   
            code.AppendLine("{");
            code.AppendLine("   public class Eval");          //产生 _Evaluator 类，所有可执行代码均在此类中运行   
            code.AppendLine("   {");
            ...
            compiled = a.CreateInstance("Asnical.Evaluator");     //通过程序集查找并声明 Asnical.Evaluator 的实例   
        ...
        {
            MethodInfo mi = compiled.GetType().GetMethod(name);//获取 _Compiled 所属类型中名称为 name 的方法的引用   
            return mi.Invoke(compiled, null);                  //执行 mi 所引用的方法   
        }
        #endregion
